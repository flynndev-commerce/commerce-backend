...
class TestProductList:
    ...
    async def test_list_products_empty(self, test_app: FastAPI, client: httpx.AsyncClient) -> None:
        """빈 상품 목록 조회 테스트"""
    ...
    async def test_list_products_pagination(self, test_app: FastAPI, client: httpx.AsyncClient) -> None:
        """상품 목록 페이지네이션 테스트"""
        headers = await create_test_seller(test_app, client)
        ...
        # 두 번째 페이지 (5개)
        response = await client.get(test_app.url_path_for(RouteName.PRODUCTS_LIST), params={"offset": 10, "limit": 10})
        response_model = BaseResponse[list[ProductRead]].model_validate(response.json())
