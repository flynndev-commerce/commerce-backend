...
class TestUserProfile:
    ...
    async def auth_headers(self, test_app: FastAPI, client: httpx.AsyncClient) -> dict[str, str]:
        """인증 헤더를 반환하는 헬퍼 메서드"""
        await create_test_user(test_app, client)
        token = await login_and_get_token(test_app, client)
        return {"Authorization": f"Bearer {token}"}

    async def test_get_current_user_success(self, test_app: FastAPI, client: httpx.AsyncClient) -> None:
        """현재 사용자 정보 조회 성공 테스트"""
        headers = await self.auth_headers(test_app, client)
    ...
    async def test_update_user_full_name(self, test_app: FastAPI, client: httpx.AsyncClient) -> None:
        """사용자 이름 수정 성공 테스트"""
        headers = await self.auth_headers(test_app, client)
    ...
    async def test_update_user_password(self, test_app: FastAPI, client: httpx.AsyncClient) -> None:
        """사용자 비밀번호 수정 성공 테스트"""
        headers = await self.auth_headers(test_app, client)
